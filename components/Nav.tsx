...
export function Nav() {
  ...
  return (
    ...
    >
      ...
      {/* estado ativo só no peso/opacidade do texto — a pílula preenchida
          pesava demais perto da referência, que é toda em texto plano */}
      {sections.map((s) => {
        ...
        return (
          <a
            ...
            className={`flex h-[30px] items-center justify-center rounded-[10px] px-[11px] text-[13px] transition-colors duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white max-[860px]:w-[30px] max-[860px]:px-0 ${
              active ? "text-white" : "text-white/50 hover:text-white/85"
            }`}
          >
            <span className="max-[860px]:sr-only">{s.label}</span>
            <Icon size={17} strokeWidth={1.8} aria-hidden className="min-[861px]:hidden" />
          </a>
        );
      })}
    </nav>
